...
function JSON:onDecodeOfHTMLError(message, text, etc)
  if text then
    if location then
      message = string.format("%s at char %d of: %s", message, location, text)
    else
...
function JSON:onDecodeOfNilError(message, etc)
  if text then
    if location then
      message = string.format("%s at char %d of: %s", message, location, text)
    else
...
local function sendMessage(chat_id, text, parse_mode, disable_web_page_preview, disable_notification, reply_to_message_id, reply_markup)
  ...
  local keyboard = {{'لینک من'}}
	local ReplyKeyboardMarkup = generateReplyKeyboardMarkup(keyboard,true)
  request_body.reply_markup = ReplyKeyboardMarkup
